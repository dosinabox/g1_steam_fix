...
!define MOD_VERSION "07.2019"
!define MOD_DETAILED_VERSION "19.7.1.0"
!define MOD_AUTHOR "D36"

Name "${MOD_NAME}"
OutFile "Gothic_Steam_Fix_(Snowball)_07.2019.exe"
...
!define MUI_HEADERIMAGE_BITMAP "logo_snowball.bmp"
!define MUI_WELCOMEFINISHPAGE_BITMAP "pic_snowball.bmp"

Caption "${MOD_NAME} (Snowball)"
...
Section "Основные патчи и обновления" SecMain
	...
	IfFileExists "$INSTDIR\system\russobit.rtf" file_found file_not_found
	file_found:
	MessageBox MB_OK "В выбранной папке уже установлен Gothic Steam Fix (Руссобит-М) или его компоненты. Для продолжения:$\n$\n- удалите игру через Steam;$\n- удалите папку игры (steam/steamapps/common/gothic);$\n- скачайте игру через Steam заново;$\n- установите данный сборник;$\n- начните новую игру (сохранения Руссобит-М не поддерживаются в версии игры от Snowball).$\n$\nУстановка прервана."
	quit
	goto end_of_test
	file_not_found:
	end_of_test:
...
SectionGroup /e "Русификация от Snowball" Group1


Section "Текст и субтитры" SecAdditional_1
	
	CreateDirectory "$INSTDIR\saves_snowball_fix\current"
	...
	SetOutPath "$INSTDIR\Data"
	File "snowball_main.vdf"

	SetOutPath "$INSTDIR\Data\ModVDF"
	File "snowball_fix.mod"

	SetOutPath "$INSTDIR"
	File "GothicModFix_readme.txt"

	SetOutPath "$INSTDIR\_work\data\scripts\_compiled"
	!insertmacro GMF_File_Rename "menu_snowball.dat" "menu.dat"
	!insertmacro GMF_File_Rename "gothic_snowball_original.dat" "gothic.dat"

	SetOutPath "$INSTDIR\_work\data\scripts\content\cutscene"
	!insertmacro GMF_File_Rename "OU_snowball_original.bin" "OU.bin"

	SetOutPath "$INSTDIR\system"
	File "snowball_fix.ini"
	File "snowball_fix.rtf"
	File "snowball.rtf"
	!insertmacro GMF_File_Rename "GothicGame_snowball.ini" "GothicGame.ini"
...
Section "Озвучка и видео" SecAdditional_2
	...
	SetOutPath "$INSTDIR\Data"
	!insertmacro GMF_File_Rename "snowball_speech.vdf" "speech.vdf"

	SetOutPath "$INSTDIR\Data\ModVDF"
	File "snowball_fix_speech.mod"

	SetOutPath "$INSTDIR\_work\data\video"
	!insertmacro GMF_File_Rename "greatPrayer_snowball.bik" "greatPrayer.bik"
	!insertmacro GMF_File_Rename "intro_snowball.bik" "intro.bik"
	!insertmacro GMF_File_Rename "playerout_snowball.bik" "playerout.bik"
...
Section "Широкоформатный монитор" SecAdditional_3
	...
	IfFileExists "$INSTDIR\Data\snowball_main.vdf" russian_found russian_not_found
	russian_found:
	SetOutPath "$INSTDIR\Data"
	File "snowball_widescreen.vdf"
	goto end_of_test2
	russian_not_found:
	SetOutPath "$INSTDIR\Data"
...
LangString DESC_SecAdditional_1 ${LANG_RUSSIAN} "Выберите эту опцию, если хотите установить русский текст и субтитры от Snowball."
LangString DESC_SecAdditional_2 ${LANG_RUSSIAN} "Выберите эту опцию, если хотите установить русскую озвучку и видео от Snowball."
